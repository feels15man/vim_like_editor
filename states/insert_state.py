...
class InsertState(State):
    ...
    def handle_backspace(self):
        if self.controller.model.cursor_x > 0:
            ...
            # self.controller.changed_lines.add(self.controller.cursor_y)
            # self.controller.model.changed_lines.update(range(self.controller.model.cursor_y, len(self.controller.model.buffer)))

    def insert_newline(self):
        current_line = self.controller.model.buffer[self.controller.model.cursor_y]

        if current_line.length() == 0:
            # Если строка пустая, левая и правая части тоже пустые
            left_part = MyString("")
            right_part = MyString("")
        elif self.controller.model.cursor_x >= current_line.length():
            # Если курсор в конце строки
        ...
        # Обновляем текущую строку и вставляем новую строку
        ...
        # Отмечаем изменённые строки
        # self.controller.model.changed_lines.update(range(self.controller.model.cursor_y - 1, len(self.controller.model.buffer)))
